...
DEFAULT_GBW = 1e6      # Hz; the simulator's stock op-amp GBW
...
# Op-amp bandwidth limit, from a single-pole op-amp model of this circuit. The
# highest frequency that still oscillates is about GBW * (ratio - 29) / 180 for
# ratios up to about 34; the 200 below adds a margin and the ratio is capped.
_MAX_MODELLED_RATIO = 34.0
_LIMIT_COEFF = 200.0
_WARN_FRACTION = 0.5   # warn from this share of the limit upward
_PULL_PERCENT = 600.0  # pull below the ladder frequency ~ 600 * f / GBW percent
...
class BandwidthLimitWarning(UserWarning):
    """The frequency is close to what the op amp's bandwidth can support."""
...
def rc_phaseshift(
    ...
    The op amp's finite bandwidth pulls the real frequency below the ladder
    value and, at high enough frequency, stops the circuit oscillating. The
    limit is estimated from a single-pole op-amp model: above it is an error,
    and from half of it upward is a BandwidthLimitWarning. Verify in the
    simulator, since the simulator's op amp may differ.
    ...
    Raises:
        ValueError: If an argument is not a finite number > 0, if the
            frequency is outside the range the capacitor list can cover, if
            the standard values give a ratio below the start-up requirement,
            or if the frequency is beyond the op amp's estimated limit.

    Warns:
        FrequencyErrorWarning: If the ladder frequency differs from the request.
        BandwidthLimitWarning: If the frequency is within a factor of 2 of the
            estimated op-amp limit.
    """
    for name, value in (
    ...
    limit = gbw * (min(ratio, _MAX_MODELLED_RATIO) - _START_RATIO) / _LIMIT_COEFF
    if actual_frequency > limit:
        raise ValueError(
            f"{actual_frequency:.4g} Hz is above the estimated {limit:.4g} Hz "
            f"that an op amp with GBW {gbw:g} Hz can sustain at Rf/R = "
            f"{ratio:.3g}; lower the frequency or raise gbw"
        )
    ...
    if actual_frequency >= _WARN_FRACTION * limit:
        pull = _PULL_PERCENT * actual_frequency / gbw
        warnings.warn(
            f"{actual_frequency:.4g} Hz is close to the estimated {limit:.4g} Hz "
            f"limit for GBW {gbw:g} Hz; expect the oscillation roughly "
            f"{pull:.1f}% below the ladder frequency, or none if the "
            "simulator's op amp is slower than the model",
            BandwidthLimitWarning,
            stacklevel=2,
        )
